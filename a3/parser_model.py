...
class ParserModel(nn.Module):
    ...
    def __init__(self, embeddings, n_features=36,
        ...
        embed_to_hidden_weight = nn.init.xavier_uniform_(
            torch.zeros(self.embed_size * self.n_features, self.hidden_size))
        embed_to_hidden_bias = nn.init.uniform_(torch.zeros(self.hidden_size))
        ...
        hidden_to_logits_weight = nn.init.xavier_uniform_(
            torch.zeros(self.hidden_size, self.n_classes))
        hidden_to_logits_bias = nn.init.uniform_(torch.zeros(self.n_classes))

        self.hidden_to_logits_weight = hidden_to_logits_weight
        self.hidden_to_logits_bias = hidden_to_logits_bias
    ...
    def forward(self, w):
        ...
        h = F.relu(x.matmul(self.embed_to_hidden_weight) +
                   self.embed_to_hidden_bias)
        h = self.dropout(h)
        logits = h.matmul(self.hidden_to_logits_weight) + \
