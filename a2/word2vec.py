...
def naiveSoftmaxLossAndGradient(
    ...
    # to integer overflow.
    probabilities = softmax(outsideVectors.dot(centerWordVec))[..., np.newaxis]
    loss = -np.log(probabilities[outsideWordIdx])

    gradCenterVec = -outsideVectors[outsideWordIdx] + \
        np.sum(outsideVectors * probabilities, axis=0)

    gradOutsideVecs = probabilities.dot(centerWordVec[..., np.newaxis].T)
    gradOutsideVecs[outsideWordIdx] = centerWordVec * \
        (probabilities[outsideWordIdx] - 1)
...
def negSamplingLossAndGradient(
    ...
    negSampleWordVecs = outsideVectors[indices[1:]]
    outsideWordVec = outsideVectors[outsideWordIdx]

    loss = -np.log(sigmoid(outsideWordVec.dot(centerWordVec))) - \
        np.sum(
            np.log(sigmoid(-negSampleWordVecs.dot(centerWordVec[..., np.newaxis]))))

    gradCenterVec = -outsideWordVec * \
        (1 - sigmoid(outsideWordVec.dot(centerWordVec))) + \
        np.sum(negSampleWordVecs *
               (1 - sigmoid(-negSampleWordVecs.dot(centerWordVec[..., np.newaxis]))), axis=0)

    gradOutsideVecs = np.zeros(outsideVectors.shape)
    gradOutsideVecs[outsideWordIdx] = -centerWordVec * \
        (1 - sigmoid(outsideWordVec.dot(centerWordVec)))
    gradOutsideVecs[negSampleWordIndices] = centerWordVec * \
        (1 - sigmoid(-negSampleWordVecs.dot(centerWordVec[..., np.newaxis])))

    # Multiply the gradient for each neg sample word that got sampled more than once
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    for outsideWord in outsideWords:
        ...
        lossW, gradCenterW, gradOutsideW = word2vecLossAndGradient(
            centerWordVec, outsideWordIdx, outsideVectors, dataset)

        loss += lossW
        gradCenterVecs[centerWordIdx] += gradCenterW
        gradOutsideVectors += gradOutsideW
